// @ts-nocheck: SE2 types resolve when deployedContracts is populated
import { useEffect, useState } from "react";
import { useIsMounted } from "usehooks-ts";
import { usePublicClient } from "wagmi";
import { useSelectedNetwork } from "@/components/scaffold-eth/hooks";
import {
  Contract,
  ContractCodeStatus,
  ContractName,
...
export function useDeployedContractInfo<TContractName extends ContractName>(
  ...
  // @ts-ignore: empty deployedContracts causes type inference issues
  const deployedContract = (contracts as any)?.[selectedNetwork.id]?.[contractName as ContractName] as Contract<TContractName>;
  const [status, setStatus] = useState<ContractCodeStatus>(ContractCodeStatus.LOADING);
  const publicClient = usePublicClient({ chainId: selectedNetwork.id as number });
